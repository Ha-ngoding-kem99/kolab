...
let state = {
    ...
    settingsView: 'tampilan', // 'profil', 'tampilan', 'keamanan', 'privasi'
    isDarkMode: false,
    currentUser: null,
...
const getFileType = (fileName) => {
    ...
    if (['png', 'jpg', 'jpeg', 'gif'].includes(extension)) return 'png';
    return 'doc'; // Default icon
};
...
const Icon = (name, className = "w-6 h-6") => {
    const icons = {
        ...
        'palette': `<path stroke-linecap="round" stroke-linejoin="round" d="M4.098 19.902a3.75 3.75 0 005.304 0l6.401-6.402a3.75 3.75 0 00-.615-6.228a2.25 2.25 0 01-3.182-3.182a3.75 3.75 0 00-6.228-.615l-6.402 6.402a3.75 3.75 0 000 5.304z" /> <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 4.5l-4.5 4.5" />`,
        'shield-check': `<path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.286zm0 13.036h.008v.008H12v-.008z" />`,
        'archive-box-x-mark': `<path stroke-linecap="round" stroke-linejoin="round" d="M8.25 7.5h7.5v3.75h-7.5V7.5z" /> <path stroke-linecap="round" stroke-linejoin="round" d="M12 15L15 12m0 0l-3-3m3 3L9 12m0 0l3 3" /> <path stroke-linecap="round" stroke-linejoin="round" d="M3 9.75a1.5 1.5 0 011.5-1.5h15a1.5 1.5 0 011.5 1.5v8.25a1.5 1.5 0 01-1.5-1.5h-15a1.5 1.5 0 01-1.5-1.5V9.75z" />`,
    };
                    ...
                    <div class="flex items-center space-x-2 sm:space-x-4">
                        <div class="relative">
                            <button data-action="toggle-user-menu" class="flex items-center space-x-2 p-1 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                                <img class="w-10 h-10 rounded-full object-cover" src="${currentUser.avatar}" alt="${currentUser.name}" />
    ...
    return `
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
            <div class="flex justify-between items-center mb-6">
                ...
                <button data-action="upload-file" class="bg-primary-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-primary-700 transition-colors flex items-center">
                    ${Icon('plus', 'w-5 h-5 mr-2')}
                    Unggah Berkas
                </button>
                <input type="file" id="file-upload-input" class="hidden" multiple accept=".pdf,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg,.gif">
            </div>
            <div class="overflow-x-auto">
                            ...
                            <tr class="bg-white dark:bg-slate-800 border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600/50">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center space-x-2">
                                        <button data-action="preview-file" data-file-id="${file.id}" class="font-medium text-xs py-1 px-2 rounded ${hasData ? 'text-primary-600 hover:text-primary-800 dark:text-primary-400 dark:hover:text-primary-300 hover:bg-slate-100 dark:hover:bg-slate-700' : 'text-slate-400 cursor-not-allowed'}" ${!hasData ? 'disabled title="Pratinjau hanya tersedia untuk berkas yang baru diunggah."' : ''}>Lihat</button>
                                        <button data-action="download-file" data-file-id="${file.id}" class="font-medium text-xs py-1 px-2 rounded ${hasData ? 'text-slate-600 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700' : 'text-slate-400 cursor-not-allowed'}" ${!hasData ? 'disabled title="Unduh hanya tersedia untuk berkas yang baru diunggah."' : ''}>Unduh</button>
                                    </div>
                                </td>
                            </tr>
                        `}).join('')}
                ...
                ${!project.files.length ? `<p class="text-center py-8 text-slate-500 dark:text-slate-400">Belum ada berkas yang diunggah.</p>` : ''}
            </div>
        </div>
    `;
}
...
function renderModals() {
    ...
    const renderModalFrame = (title, content, isOpen = true, sizeClass = 'max-w-lg') => {
        if (!isOpen) return '';
        return `
            <div class="fixed inset-0 bg-black/60 z-50 flex justify-center items-center" data-action="close-modal">
                <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full ${sizeClass} m-4" data-action="modal-content">
    ...
    switch(modal) {
        ...
        case 'settings':
            const { settingsView, isDarkMode, currentUser: u } = state;
            const settingsNavItems = [
                { id: 'profil', name: 'Akun & Profil', icon: 'user' },
                { id: 'tampilan', name: 'Tampilan & Tema', icon: 'palette' },
                { id: 'keamanan', name: 'Keamanan', icon: 'shield-check' },
                { id: 'privasi', name: 'Privasi dan Data', icon: 'archive-box-x-mark' },
            ];
            ...
            switch(settingsView) {
                case 'profil':
                    settingsContent = `
                        <h4 class="text-xl font-bold mb-1">Informasi Akun</h4>
                        <p class="text-slate-500 dark:text-slate-400 mb-6">Informasi dasar Anda yang ditampilkan di aplikasi.</p>
                        <div class="space-y-4">
                             <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Nama Lengkap</label>
                                <p class="mt-1 text-slate-900 dark:text-slate-100">${u.name}</p>
                            </div>
                             <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">NIM</label>
                                <p class="mt-1 text-slate-900 dark:text-slate-100">${u.nim}</p>
                            </div>
                             <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Email</label>
                                <p class="mt-1 text-slate-900 dark:text-slate-100">${u.email}</p>
                            </div>
                        </div>
                         <div class="mt-6 border-t dark:border-slate-700 pt-4">
                             <button data-action="open-profile-from-settings" class="text-sm font-semibold bg-primary-600 text-white hover:bg-primary-700 py-2 px-3 rounded-md transition-colors">
                                Edit Profil
                            </button>
                        </div>
                    `;
                ...
                case 'tampilan':
                    settingsContent = `
                        <h4 class="text-xl font-bold mb-1">Tampilan & Tema</h4>
                        <p class="text-slate-500 dark:text-slate-400 mb-6">Sesuaikan tampilan aplikasi sesuai preferensi Anda.</p>
                        <div class="flex justify-between items-center p-4 rounded-lg bg-slate-50 dark:bg-slate-700/50">
                            <span class="font-medium text-slate-700 dark:text-slate-300">Mode Gelap</span>
                            <button data-action="toggle-dark-mode" class="relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none ${isDarkMode ? 'bg-primary-600' : 'bg-slate-300'}">
                                <span class="inline-block w-5 h-5 rounded-full bg-white shadow-lg transform ring-0 transition ease-in-out duration-200 ${isDarkMode ? 'translate-x-5' : 'translate-x-0'}"></span>
                            </button>
                        </div>
                    `;
                    break;
                case 'keamanan':
                    settingsContent = `
                        <h4 class="text-xl font-bold mb-1">Keamanan</h4>
                        <p class="text-slate-500 dark:text-slate-400 mb-6">Ubah password Anda secara berkala untuk menjaga keamanan akun.</p>
                        <form data-action="change-password">
                            <div class="space-y-4">
                                <div>
                                    <label for="old-password" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Password Lama</label>
                                    <input type="password" name="old-password" required class="mt-1 block w-full rounded-md border-slate-300 dark:border-slate-600 shadow-sm bg-white dark:bg-slate-700" />
                                </div>
                                <div>
                                    <label for="new-password" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Password Baru</label>
                                    <input type="password" name="new-password" required class="mt-1 block w-full rounded-md border-slate-300 dark:border-slate-600 shadow-sm bg-white dark:bg-slate-700" />
                                </div>
                            </div>
                            <div class="mt-6 border-t dark:border-slate-700 pt-4">
                                <button type="submit" class="text-sm font-semibold bg-primary-600 text-white hover:bg-primary-700 py-2 px-3 rounded-md transition-colors">
                                    Ubah Password
                                </button>
                            </div>
                        </form>
                ...
                case 'privasi':
                    settingsContent = `
                        <h4 class="text-xl font-bold mb-1">Privasi dan Data</h4>
                        <p class="text-slate-500 dark:text-slate-400 mb-6">Kelola data Anda yang tersimpan di aplikasi ini.</p>
                        <div class="space-y-4">
                             <div class="flex justify-between items-center p-4 rounded-lg bg-slate-50 dark:bg-slate-700/50">
                                <div>
                                    <h5 class="font-semibold">Ekspor Data Anda</h5>
                                    <p class="text-sm text-slate-500 dark:text-slate-400">Unduh semua data proyek dan pengguna dalam format JSON.</p>
                                </div>
                                <button data-action="export-data" class="text-sm font-semibold border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-600 py-2 px-3 rounded-md transition-colors">
                                    Ekspor
                                </button>
                            </div>
                             <div class="flex justify-between items-center p-4 rounded-lg border border-red-300 dark:border-red-600/50 bg-red-50 dark:bg-red-900/20">
                                <div>
                                    <h5 class="font-semibold text-red-800 dark:text-red-300">Hapus Akun</h5>
                                    <p class="text-sm text-red-600 dark:text-red-400">Tindakan ini akan menghapus akun Anda dan semua proyek yang Anda miliki secara permanen.</p>
                                </div>
                                <button data-action="delete-account" class="text-sm font-semibold bg-red-600 text-white hover:bg-red-700 py-2 px-3 rounded-md transition-colors">
                                    Hapus
                                </button>
                            </div>
                        </div>
            ...
            modalContent = renderModalFrame('Pengaturan', `
                <div class="flex flex-col sm:flex-row">
                    <nav class="w-full sm:w-1/3 p-4 border-b sm:border-b-0 sm:border-r border-slate-200 dark:border-slate-700">
                        <ul class="space-y-1">
                            ${settingsNavItems.map(item => `
                                <li>
                                    <a href="#" data-action="set-settings-view" data-view="${item.id}" class="flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors ${settingsView === item.id ? 'bg-primary-100 dark:bg-primary-900/50 text-primary-700 dark:text-primary-300' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'}">
                                        ${Icon(item.icon, 'w-5 h-5 mr-3')}
                                        ${item.name}
                                    </a>
                                </li>
                 ...
                 <div class="bg-slate-50 dark:bg-slate-900/50 px-4 py-3 sm:px-6 flex justify-end rounded-b-lg border-t dark:border-slate-700">
        ...
        case 'previewFile':
            ...
            if (file.mimeType && file.mimeType.startsWith('image/')) {
                previewContent = `<img src="${file.dataUrl}" alt="${file.name}" class="max-w-full max-h-[80vh] mx-auto object-contain">`;
            } else if (file.mimeType === 'application/pdf') {
                previewContent = `<iframe src="${file.dataUrl}" class="w-full h-[80vh]" title="${file.name}"></iframe>`;
            } else {
                previewContent = `
                    <div class="p-8 text-center">
                        <h4 class="text-lg font-semibold text-slate-800 dark:text-slate-100">Pratinjau Tidak Tersedia</h4>
                        <p class="mt-2 text-slate-500 dark:text-slate-400">Pratinjau tidak didukung untuk tipe berkas ini.<br>Silakan unduh berkas untuk melihat isinya.</p>
                        <button data-action="download-file" data-file-id="${file.id}" class="mt-6 bg-primary-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-primary-700 transition-colors">
                            Unduh Berkas
                        </button>
                    </div>`;
            }
            
            modalContent = renderModalFrame(`Pratinjau: ${file.name}`, `
...
function handleDeleteAccount() {
    ...
    const confirmation = confirm(
        ...
        'Semua proyek yang Anda miliki juga akan dihapus. Tindakan ini tidak dapat diurungkan.'
    );

    if (confirmation) {
        const userIdToDelete = state.currentUser.id;

        // 1. Remove user from allUsers list
        state.allUsers = state.allUsers.filter(u => u.id !== userIdToDelete);
        saveToStorage(USERS_STORAGE_KEY, state.allUsers);

        // 2. Filter projects:
        // - Remove projects owned by the user.
        // - Remove the user from member lists of other projects.
        // - Unassign user from tasks in remaining projects.
        const updatedProjects = state.projects
            .filter(p => p.adminId !== userIdToDelete) // Remove projects they own
            .map(p => {
                const newMembers = p.members.filter(m => m.id !== userIdToDelete);
                const newTasks = p.tasks.map(t => {
                    if (t.assigneeId === userIdToDelete) {
                        return { ...t, assigneeId: null };
                    }
                    return t;
                });
                return { ...p, members: newMembers, tasks: newTasks };
            });

        state.projects = updatedProjects;
        saveToStorage(PROJECTS_STORAGE_KEY, state.projects);

        // 3. Reset state to log out
        state.currentUser = null;
        state.selectedProjectId = null;
        state.modal = null;
        state.modalData = {};

        // 4. Re-render the app to show the auth screen
        renderApp();
    }
}
...
function handleFileUpload(files) {
    ...
    for (const file of files) {
        ...
        reader.onload = (event) => {
            ...
            if (filesToProcess === 0) {
                saveToStorage(PROJECTS_STORAGE_KEY, state.projects);
                setProjectView('files'); // Refresh the files view
            }
        };
...
function setProjectView(view) {
    ...
    const project = state.projects.find(p => p.id === state.selectedProjectId);
    if (!project) return renderApp(); // Go back to dashboard if project not found
...
appContainer.addEventListener('click', (e) => {
    ...
    const { action, modal, projectId, view, taskId, status, code, fileId } = actionTarget.dataset;
    ...
    switch(action) {
        ...
        case 'open-modal':
            state.modal = modal;
            if(modal === 'settings') state.settingsView = 'tampilan'; // Reset to default tab
            if(projectId) {
                state.modalData = { project: state.projects.find(p => p.id === projectId) };
            }
            renderApp();
            break;
        case 'open-profile-from-settings':
            state.modal = 'profile';
            renderApp();
            break;
        case 'close-modal':
            if (target.closest('[data-action="modal-content"]') && !target.closest('button[data-action="close-modal"]')) return;
            state.modal = null;
            state.modalData = {};
            renderApp();
            break;
        ...
        case 'set-project-view':
            setProjectView(view);
            break;
        case 'set-settings-view':
            state.settingsView = view;
            renderApp(); // Re-render only the modal content
            break;
        ...
        case 'preview-file': {
            ...
            if (file && file.dataUrl) {
                state.modal = 'previewFile';
                state.modalData = { file };
                renderApp();
            }
            break;
        }
...
appContainer.addEventListener('change', (e) => {
    ...
    if (target.id === 'file-upload-input' && target.files) {
        handleFileUpload(target.files);
        target.value = ''; // Reset input so the same file can be uploaded again
    }
});
